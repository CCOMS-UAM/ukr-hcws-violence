...
ragg::agg_png("out/plots/dose_resp_plots.png",
              width = 170,
              height = 170,
              units = "mm",
              res = 600,
...
brm_results_df_agg <- 
  ...
  if(nrow(target_row) > 0) {
    ...
    if (raw_col_name %in% names(dat)) {
      ...
      # Extract vectors
      x_raw <- as.character(dat[[raw_col_name]])
...
ragg::agg_png("out/plots/forest_bayes_comparison.png",
              width = 210,
              height = 170,
              units = "mm",
              res = 600,
...
ragg::agg_png("out/plots/bayes_diagnostic_plots.png",
              width = 220,
              height = 160,
              units = "mm",
              res = 600,
...
ragg::agg_png("out/plots/forest_sens_comparison.png",
              width = 210,
              height = 170,
              units = "mm",
              res = 600,
...
ragg::agg_png("out/plots/forest_sens_comparison_suic.png",
              width = 210,
              height = 170,
              units = "mm",
              res = 600,
...
map_info_prev_exp <- 
  ...
  left_join(ua_reg_conflict |> select(fips_code, combat, occupation),
             by = "fips_code") |> 
  mutate(
    conflict_status = case_when(
      combat == "Yes" | occupation %in% c("Yes", "Partial") ~ "Active",
      TRUE ~ "Non-active"
    ),
    prev_bin = cut(
      Prevalence,
...
map_info_prev_out <- 
  ...
  left_join(
    ua_reg_conflict |> select(fips_code, combat, occupation),
    by = "fips_code"
  ) |> 
  mutate(
    conflict_status = case_when(
...
outcomes_gen_map <-
  ...
  geom_sf_pattern(
    aes(
      fill = prev_bin,
      colour = conflict_status,
      linewidth = I(ifelse(conflict_status == "Active", 0.8, 0.3)),
      pattern = conflict_status
    ),
  ...
  guides(
    ...
    color = guide_legend(override.aes = list(
      fill = c("white", "white"),
      pattern = c("crosshatch", "none"),       # Force the stripe pattern into the legend
      pattern_color = c("grey60", NA),  # Match your map's stripe color
      pattern_fill = c("grey60", NA),       
      pattern_density = c(0.05, 0),
      pattern_spacing = c(0.015, 1)
    ))
  ) +
  geom_sf_text(
    aes(
...
ragg::agg_png("out/plots/outcomes_map.png",
              width = 210,
              height = 160,
              units = "mm",
              res = 600,
...
exposures_gen_map <-
  ...
  guides(
    ...
    color = guide_legend(override.aes = list(
      fill = c("white", "white"),         
      pattern = c("crosshatch", "none"),      
      pattern_color = c("grey60", NA),    
      pattern_fill = c("grey60", NA),
      pattern_density = c(0.05, 0),        
      pattern_spacing = c(0.015, 1)        
    ))
  ...
  theme(
    ...
    legend.position = "right", 
    legend.title = element_text(face = "bold"),
    strip.text = element_text(size = 10, face = "bold", margin = margin(b = 2, t = 2))
  )
...
ragg::agg_png("out/plots/exposures_map.png",
              width = 210,
              height = 160,
              units = "mm",
              res = 600,
...
```{r reg-dep-maps-prep-prop, eval = FALSE}
...
reg_dep_har <- 
  ...
  mutate(
    prob_No = plogis(b_Intercept + r_loc_3),
    prob_Yes = plogis(b_Intercept + r_loc_3 + b_work_30_dicYes),
    PR = prob_Yes / prob_No # PR per draw
    ) |>
  group_by(loc_3) |> 
  summarise(
    PR = mean(PR) # Posterior mean of the PR
    ) |> 
  mutate(loc_3 = str_replace_all(loc_3, "\\.", " ")) |>
  mutate(loc_3 = case_when(
           loc_3 == "М  Київ" ~ "М. Київ",
  ...
  add_row(tibble_row(loc_3 = "Ар Крим")) |> 
  left_join(ua_reg_conflict |> select(loc_3, loc_en, fips_code, combat, occupation),
            by = "loc_3") |> 
  mutate(
    Exposure = "Sexual Harassment",
    Outcome = "Depression"
...
reg_dep_bul <- 
  ...
  mutate(
    prob_No = plogis(b_Intercept + r_loc_3),
    prob_Yes = plogis(b_Intercept + r_loc_3 + b_work_33_dicYes),
    PR = prob_Yes / prob_No
    ) |>
  group_by(loc_3) |> 
  summarise(
...
reg_dep_threats <- 
  ...
  mutate(
    prob_No = plogis(b_Intercept + r_loc_3),
    prob_Yes = plogis(b_Intercept + r_loc_3 + b_work_31_dicYes),
    PR = prob_Yes / prob_No
    ) |> 
  group_by(loc_3) |> 
  summarise(
...
reg_dep_viol <- 
  ...
  mutate(
    prob_No = plogis(b_Intercept + r_loc_3),
    prob_Yes = plogis(b_Intercept + r_loc_3 + b_work_32_dicYes),
    PR = prob_Yes / prob_No
    ) |>
  group_by(loc_3) |> 
  summarise(
...
saveRDS(reg_dep_exp, "out/dataframes/region_depression_assoc_prs.rds")
writexl::write_xlsx(reg_dep_exp,
                    "out/dataframes/region_depression_assoc_prs.xlsx")
```

### Anxiety

```{r reg-anx-maps-prep-prob, eval = FALSE}
bay_anx_har_agg_adj <-
...
saveRDS(reg_anx_exp, "out/dataframes/region_anxiety_assoc_prs.rds")
writexl::write_xlsx(reg_anx_exp,
                    "out/dataframes/region_anxiety_assoc_prs.xlsx")
```

### Maps

```{r map generation-assoc-prevs}
...
ragg::agg_png("out/plots/asssoc_dep_map.png",
              width = 210,
              height = 160,
              units = "mm",
              res = 600,
...
ragg::agg_png("out/plots/asssoc_anx_map.png",
              width = 210,
              height = 160,
              units = "mm",
              res = 600,
...
print(anx_assoc_map_grid)

dev.off()
```

```{r map generation-assoc-prs}

reg_dep_exp <- readRDS("out/dataframes/region_depression_assoc_prs.rds")
reg_anx_exp <- readRDS("out/dataframes/region_anxiety_assoc_prs.rds")
...
# Build map info and add conflict indicator
map_info_assoc <- 
  ukraine |>
  select(fips, geometry) |>   
  bind_rows(crimea) |> 
...
pr_assoc_map_grid <-
  ...
  mutate(
    PR_bin = cut(
      PR, 
      # Narrower breaks to capture the actual variance in your data
      breaks = c(1.2, 1.4, 1.6, 1.8, 2.0), 
      labels = c("1.2 - 1.4", "1.4 - 1.6", "1.6 - 1.8", "1.8 - 2.0"),
      include.lowest = TRUE
    ),
    Exposure = factor(Exposure, levels = exposure_order),
    Outcome = factor(Outcome, levels = c("Depression", "Anxiety"))
  ) |> 
  arrange(Exposure) |>
  ggplot() +
  geom_sf_pattern(
    ...
    pattern_color = "grey70",
    pattern_fill = NA, # keeps inside transparent
    pattern_alpha = 0.3, 
    pattern_density = 0.1,
  ...
  guides(
    ...
    color = guide_legend(override.aes = list(
      fill = "white",         
      pattern_alpha = 1, # Makes the pattern fully opaque in the little box
      pattern_density = 0.5    # Squeezes the lines closer together
    ))
  ) +
  coord_sf(expand = FALSE) + # removes internal padding between map and box
  facet_grid(rows = vars(Exposure),
             cols = vars(Outcome)) +
  labs(color = "Conflict status",
       linewidth = "Conflict status") +
  theme_void() + 
  theme(
    # Set top, right, bottom, and left margins to absolutely zero
    plot.margin = margin(t = 0, r = 0.3, b = 0, l = 0.3, unit = "cm"),
    # Keep the facets close together
    panel.spacing = unit(0.2, "lines"),
    legend.position = "left",
    legend.title = element_text(face = "bold"),
    strip.text = element_text(size = 11, face = "bold", margin = margin(b = 2, t = 2))
  )
...
# grouped_pr_table <- 
  # map_info_assoc |>
  # st_drop_geometry() |> # Remove spatial data for table calculation
  # filter(!is.na(PR)) |>
  # group_by(Outcome, Exposure, conflict_status) |>
  # summarise(Mean_PR = mean(PR), .groups = "drop") |>
  # pivot_wider(names_from = conflict_status, values_from = Mean_PR) |>
  # mutate(across(where(is.numeric), ~round(., 2)))

  
  ggsave(
  "pr_assoc_map.pdf",
  plot = pr_assoc_map_grid,
  path = "out/plots/",
  scale = 1.7,
  width = 170,
  height = 180,
...
ragg::agg_png("out/plots/pr_asssoc_map.png",
              width = 160,
              height = 210,
              units = "mm",
              res = 600,
...
ragg::agg_png("out/plots/missing_vals_through.png",
              width = 210,
              height = 170,
              units = "mm",
              res = 600,
